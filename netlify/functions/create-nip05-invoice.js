...
async function isHandleTaken(handle) {
  ...
  if (!token) return false; // fail open — webhook is the final guard
...
exports.handler = async function (event) {
  ...
  if (!handle || !HANDLE_REGEX.test(handle)) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Invalid handle format. Use lowercase letters, numbers, hyphens, and underscores (1-32 characters).' }) };
  }

  if (RESERVED_HANDLES.includes(handle)) {
    return { statusCode: 400, body: JSON.stringify({ error: 'This handle is reserved and cannot be purchased.' }) };
  }

  // Validate npub
  if (!npub || !/^npub1[a-z0-9]{58}$/.test(npub)) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Invalid npub format. It should start with npub1 and be 63 characters long.' }) };
  }

  var hex = npubToHex(npub);
  if (!hex) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Could not decode npub. Please check and try again.' }) };
  }

  // Validate amount
  if (!amount || amount < 5 || amount > 10000) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Amount must be between $5 and $10,000.' }) };
  }

  // Double-check availability
  if (await isHandleTaken(handle)) {
    return { statusCode: 409, body: JSON.stringify({ error: 'This handle has already been taken. Please choose a different one.' }) };
  }
